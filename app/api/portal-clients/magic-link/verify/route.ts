...
export const POST = withErrorHandler('portal-clients/magic-link/verify POST', async (req) => {
  ...
  // Delivery step 4 ("Portal Access"). Awaited, not fire-and-forget: the
  // serverless instance freezes once the response is sent, so an un-awaited
  // write here would often simply be dropped. advanceDeliveryStep is
  // forward-only, so repeat logins cost one cheap SELECT and change nothing.
  await onPortalFirstLogin(client)

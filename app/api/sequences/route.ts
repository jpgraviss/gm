...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await db
    ...
    .insert({
      ...
      send_via:         body.sendVia ?? 'gmail',
      from_name:        body.fromName ?? null,
      from_email:       body.fromEmail ?? null,
      assigned_rep_id:  body.assignedRepId ?? null,
      daily_send_limit: body.dailySendLimit ?? 200,
      per_minute_limit: body.perMinuteLimit ?? 3,
      send_window_start: body.sendWindowStart ?? 8,
      send_window_end:  body.sendWindowEnd ?? 18,
      send_on_weekends: body.sendOnWeekends ?? false,
      timezone:         body.timezone ?? 'America/New_York',
      thread_mode:      body.threadMode ?? true,

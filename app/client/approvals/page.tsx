...
// Ported from app/portal/approvals/page.tsx as part of the /portal -> /client
// merge (Batch 2). Same data/API calls as the portal original; chrome
// (company identity header, notifications, sign-out, and the "back to
// portal" link — now the CLIENT_ROUTES nav strip in app/client/layout.tsx)
// comes from that shared layout instead of being reimplemented here.
//
// AUDIT #155 fix: the portal original rendered a full e-signature capture
// UI (typed cursive name OR a drawn canvas signature) but handleAccept()
// discarded the captured signature entirely, only ever PATCHing
// {status, clientSigned/respondedDate}. This version actually sends the
// captured signature (signatureData/signatureType) to the PATCH routes,
// which now persist it on contracts/proposals — mirroring, in shape, the
// app's real e-signature system (signature_requests.signature_data, used
// by the public /sign/[token] flow).
...
export default function ClientApprovalsPage() {
  ...
  async function handleAccept() {
    ...
    // being thrown away — signatureType records which capture mode was
    // used so signatureData can be rendered/audited correctly later.
    const signatureType = sigMode === 'type' ? 'typed' : 'drawn'
    const signatureData = sigMode === 'type' ? typedSig.trim() : (drawnSig as string)

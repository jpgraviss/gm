...
export default function PortalSeoServicePage() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([seoData, kwData]) => {
        if (seoData?.strategy) {
          setStrategy({ ...DEFAULT_STRATEGY, ...seoData.strategy })
        }
        if (Array.isArray(kwData)) {
          setTrackedKeywords(
            kwData
              .filter((k: TrackedKw) => k.portalVisible !== false)
              .map((k: TrackedKw) => ({
                keyword: k.keyword,
                currentPosition: k.currentPosition,
                previousPosition: k.previousPosition,
                portalVisible: k.portalVisible,
              })),
          )
        }
      })
  ...
  const allKeywords: KeywordRanking[] = useMemo(() => {
    const fromStrategy = strategy.keywords ?? []
    const fromTracker = trackedKeywords
      .filter(k => !fromStrategy.some(s => s.keyword.toLowerCase() === k.keyword.toLowerCase()))
      .map(k => ({
        keyword: k.keyword,
        currentRank: k.currentPosition ?? 0,
        previousRank: k.previousPosition ?? 0,
      }))
    return [...fromStrategy, ...fromTracker]
  }, [strategy.keywords, trackedKeywords])
        ...
        {allKeywords.length > 0 && (
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
              ...
              <span className="text-xs text-gray-400">{allKeywords.length} keywords</span>
            </div>

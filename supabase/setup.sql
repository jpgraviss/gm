-- ─────────────────────────────────────────────────────────────────────────────
-- GravHub — Complete Setup Script (idempotent — safe to re-run)
...
DO $$ BEGIN
  ...
  DROP POLICY IF EXISTS "auth_self_insert_team_members"  ON public.team_members;
  -- crm
...
-- Create the client-files bucket for file uploads (used by /api/files)
...
-- Storage RLS: allow authenticated users to read/upload/delete files
DO $$ BEGIN
  DROP POLICY IF EXISTS "auth_read_client_files"   ON storage.objects;
  DROP POLICY IF EXISTS "auth_insert_client_files" ON storage.objects;
  DROP POLICY IF EXISTS "auth_update_client_files" ON storage.objects;
  DROP POLICY IF EXISTS "auth_delete_client_files" ON storage.objects;
EXCEPTION WHEN undefined_table THEN NULL;
END $$;

CREATE POLICY "auth_read_client_files"
  ON storage.objects FOR SELECT TO authenticated
  USING (bucket_id = 'client-files');

CREATE POLICY "auth_insert_client_files"
  ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'client-files');

CREATE POLICY "auth_update_client_files"
  ON storage.objects FOR UPDATE TO authenticated
  USING (bucket_id = 'client-files');

CREATE POLICY "auth_delete_client_files"
...
INSERT INTO public.app_settings (id, company, pipeline_stages, service_types, contact_tags)
VALUES (
  'global',
  '{"name": "Graviss Marketing", "email": "info@gravissmarketing.com"}',
  '[{"id": "lead", "label": "Lead"}, {"id": "qualified", "label": "Qualified"}, {"id": "proposal", "label": "Proposal"}, {"id": "negotiation", "label": "Negotiation"}, {"id": "closed-won", "label": "Closed Won"}, {"id": "closed-lost", "label": "Closed Lost"}]',
  '[{"id": "website", "label": "Website"}, {"id": "seo", "label": "SEO"}, {"id": "ppc", "label": "PPC"}, {"id": "social", "label": "Social Media"}, {"id": "branding", "label": "Branding"}, {"id": "general", "label": "General"}]',
  '[{"id": "vip", "label": "VIP"}, {"id": "decision-maker", "label": "Decision Maker"}, {"id": "referral", "label": "Referral"}]'
)
ON CONFLICT (id) DO NOTHING;


-- ═══════════════════════════════════════════════════════════════════════════════
-- DONE! All tables, RLS policies, storage, and seed data are set up.
-- ═══════════════════════════════════════════════════════════════════════════════
